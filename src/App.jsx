...
function App() {
  ...
  const loadSignaturesFromServer = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to load signatures from server, status:', response.status);
        setSignatures([]);
      }
    } catch (error) {
  ...
  const handleSignatureSave = async (signatureData) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to save signature to server, status:', response.status);
        alert('Failed to save signature. Please try again.');
      }
    } catch (error) {
      console.error('Error saving signature to server:', error);
      alert('Error saving signature. Please check your connection and try again.');
    }
  };
  ...
  const clearAllSignatures = async () => {
    try {
      ...
      alert('Error clearing signatures. Please check your connection and try again.');
    }
  };

  // Check if user is HR or Board member
  const isHROrBoard = user && (user.userType === 'hr' || user.userType === 'board');
          ...
          <div className="container mx-auto">
            ...
            <main className="space-y-12">
              <div className="transform animate-slide-up">
                <SignatureCanvas user={user} onSignatureSave={handleSignatureSave} />
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-neutral-border/20 transform animate-slide-up animation-delay-100">
                ...
                <div className="bg-gradient-to-r from-neutral-light to-white rounded-xl p-6 mb-6">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-2xl font-bold text-brand">{signatures.length} / {employees.length}</span>
                  </div>
                  ...
                  <div className="w-full bg-neutral-border rounded-full h-4 mb-4">
                    <div 
                      ...
                      style={{ width: `${employees.length > 0 ? Math.round((signatures.length / employees.length) * 100) : 0}%` }}
                    ></div>
                  </div>
                  
                  <div className="text-center">
                    <span className="text-3xl font-bold text-extra-seagreen">
                      {employees.length > 0 ? Math.round((signatures.length / employees.length) * 100) : 0}%
                    </span>
                    <span className="text-neutral-dark ml-2">Complete</span>
                  </div>
                </div>

                {employees.length > 0 && signatures.length === employees.length && (
              ...
              {user.email === 'krutik@weetechsolution.com' && (
            ...
            <footer className="mt-16 text-center transform animate-fade-in animation-delay-600">
              <div className="inline-block p-4 bg-white/60 backdrop-blur-sm rounded-lg border border-neutral-border shadow-sm">
                <p className="text-neutral-dark/70">© {new Date().getFullYear()} Employee Trip Signature Campaign</p>
              </div>
            </footer>
          </div>
        </div>
