...
const SignatureGallery = ({ signatures, employees, currentUserEmail }) => {
  ...
  const sendToHR = async () => {
    ...
    try {
      ...
      if (validSignatures.length === 0) {
        ...
        alert(
          "No valid signatures to send. Please ensure signatures are available."
        );
        setIsSending(false);
        return;
      }
      ...
      alert(
        `Error sending email: ${error.message}. Please check your connection and try again.`
      );
    } finally {
      setIsSending(false);
    }
  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      ...
      {allEmployeesSigned && (
        <div className="mb-8 p-6 bg-gradient-to-r from-extra-seagreen/10 via-brand/10 to-extra-turquoise/10 border-2 border-extra-seagreen/30 rounded-2xl text-center animate-bounce-subtle">
          <div className="flex items-center justify-center mb-2">
            ...
            <p className="text-2xl font-bold text-extra-seagreen">
              All WeeTechies have signed the petition!
            </p>
        ...
        <div className="mb-8 flex justify-center">
          {!sendSuccess ? (
            <button
              onClick={sendToHR}
              disabled={isSending}
              className={`bg-gradient-to-r from-brand to-extra-turquoise hover:from-brand-dark hover:to-extra-seagreen text-white font-bold py-4 px-8 rounded-2xl text-lg transition-all duration-300 transform hover:scale-105 hover:shadow-xl active:scale-95 focus:outline-none focus:ring-4 focus:ring-brand/30 ${
                isSending
            ...
            <div className="p-6 bg-gradient-to-r from-extra-seagreen/10 to-brand/10 border-2 border-extra-seagreen/30 rounded-2xl text-center w-full max-w-md animate-scale-in">
              <div className="flex items-center justify-center mb-2">
                <svg
                  className="w-8 h-8 text-extra-seagreen mr-2"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
                ...
                <p className="font-bold text-extra-seagreen text-lg">
                  Email sent successfully!
                </p>
              </div>
              <p className="text-neutral-dark">
                HR and Board Directors have been notified.
              </p>
            </div>
          )}
        </div>
      ...
      {(!Array.isArray(signatures) ||
        signatures.length === 0 ||
        !signatures.some((sig) => sig && sig.signature)) && (
        <div className="text-center p-12 bg-gradient-to-br from-neutral-light/50 to-white rounded-2xl border-2 border-dashed border-neutral-border animate-pulse-gentle">
          ...
          <p className="text-xl text-neutral-dark font-semibold mb-2">
            No signatures yet
          </p>
          <p className="text-neutral-dark/60">
            Be the first to sign and start the campaign!
          </p>
        </div>
      )}
    </div>
