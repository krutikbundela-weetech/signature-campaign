...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    const allowedOriginStrings = corsOrigins.split(',').map(url => url.trim());
    
    // Create regex patterns for localhost and IP ranges (for development)
    const allowedOrigins = [
      ...allowedOriginStrings.map(url => new RegExp(`^${url.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`)),
      /^http:\/\/localhost:\d+$/,
      /^http:\/\/127\.0\.0\.1:\d+$/,
      /^http:\/\/192\.168\.\d+\.\d+:\d+$/
    ];
...
const savedSignaturesDir = path.join(__dirname, process.env.SIGNATURES_DIR || 'public/SavedSignatures');
...
app.delete('/api/clear-signatures', (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'All signatures cleared successfully' });
  } catch (error) {
...
const loadHRAndBoardEmails = () => {
  try {
    const employeesFilePath = path.join(__dirname, process.env.EMPLOYEES_FILE || 'public/employees.json');
    const employeesData = JSON.parse(fs.readFileSync(employeesFilePath, 'utf8'));
    
    // Extract HR emails
    const hrEmails = employeesData.hrBoard?.hr?.map(hr => hr.email).filter(email => email) || [];
    
    // Extract Board emails
    const boardEmails = employeesData.hrBoard?.board?.map(board => board.email).filter(email => email) || [];
    ...
    console.error('Error loading HR and Board emails from employees.json:', error);
...
app.post('/api/send-email', async (req, res) => {
  console.log('==== Received request to send email to HR and Board Directors ====');
  ...
  try {
    ...
    if (!req.body.signatures) {
      ...
      return res.status(400).json({ error: 'Invalid request. Signatures data is required.' });
    }
    
    const signaturesCount = Array.isArray(req.body.signatures) ? req.body.signatures.length : 0;
    ...
    if (hrEmails.length === 0) {
      ...
      return res.status(500).json({ error: 'No HR emails configured. Please check employees.json file.' });
    }
    ...
    const employeeNames = req.body.signatures.map(sig => sig.name || sig.email).join(', ');
    
    // Create a funny email message
    const emailSubject = '🏝️ URGENT: Team Trip Request - Signed by Everyone! 🏝️';
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
        ...
        <div style="text-align: center; margin-top: 30px; font-size: 12px; color: #888;">
          <p>This email was automatically generated from our ${process.env.APP_NAME || 'signature campaign'} application.</p>
        </div>
      </div>
    `;
    ...
    const mailOptions = {
      from: `"${process.env.APP_NAME || 'Signature Campaign'}" <${process.env.EMAIL_USER || 'tasks410@gmail.com'}>`,
...
app.listen(PORT, () => {
  ...
  console.log(`Frontend URL: ${process.env.VITE_FRONTEND_URL || 'http://localhost:5173'}`);
});